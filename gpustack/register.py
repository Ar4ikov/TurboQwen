...
Four calls the UI makes for you and the raw API does not: the backend (from YAML), the
model, the *model route* that makes the model visible to the /v1 gateway, and the
*metrics mapping* that makes the instance visible to Grafana. A model created through
POST /v2/models alone answers on its backend port but the gateway says "Model not found"
until a route with a target on it exists; and the worker aggregates an instance's
/metrics only when its backend name has a runtime_mapping entry in the metrics config
(builtin: vLLM, SGLang, MindIE), so a custom backend reads "No data" on the GPUStack
Model dashboard until one is registered. Every step is idempotent.
...
# 4. metrics mapping: the image serves vLLM's own metric names, so the builtin vLLM "*"
#    mapping is registered under this backend's name. POST /v2/metrics/config writes
#    <data_dir>/custom_metrics_config.yaml on the server, which then replaces the builtin
#    file (so it is re-read from the builtin one here, not hand-written); the worker
#    refreshes its copy within 5 minutes.
st, cfg = call("GET", "/v2/metrics/config")
st2, default = call("GET", "/v2/metrics/default-config")
if st == 200 and st2 == 200 and isinstance(cfg, dict) and isinstance(default, dict):
    rm = cfg.setdefault("runtime_mapping", {})
    vllm_star = ((default.get("runtime_mapping") or {}).get("vLLM") or rm.get("vLLM") or {}).get("*")
    wanted = {"*": dict(vllm_star)} if vllm_star else None
    if not wanted:
        print("metrics mapping: no vLLM entry in the builtin metrics config, skipped")
    elif rm.get(name) == wanted:
        print(f"metrics mapping {name}: exists")
    else:
        rm[name] = wanted
        st, r = call("POST", "/v2/metrics/config", cfg)
        print(f"metrics mapping {name}: {st} {'registered; the worker picks it up within 5 min' if st == 200 else r}")
else:
    print(f"metrics mapping: GET /v2/metrics/config -> {st}, default-config -> {st2}; skipped")
